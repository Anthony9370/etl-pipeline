...
CHECKPOINT_FILE = "connected_accounts_checkpoint.txt"
...
def extract_connected_accounts():
    query = """
        SELECT id, connected_account_id, merchant_account_id, name, email, country,
               charge_type, is_active, first_seen_at, last_seen_at, created_at, updated_at
...
if __name__ == "__main__":
    ...
    for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=chunksize)):
        ...
        # Optionally: chunk.to_csv(f"connected_accounts_chunk_{i+1}.csv", index=False)
    logging.info(f"Extraction complete. Total rows extracted: {total_rows}")
