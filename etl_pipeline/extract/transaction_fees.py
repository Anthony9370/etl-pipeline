...
def extract_by_date():
    ...
    rng = pd.read_sql_query("SELECT MIN(transaction_date) AS min_date, MAX(transaction_date) AS max_date FROM transaction_fees", engine)
    start = pd.to_datetime(rng.loc[0, "min_date"])
    end = pd.to_datetime(rng.loc[0, "max_date"])
    if pd.isna(start) or pd.isna(end):
        logging.info("No data in transaction_fees")
        return
    ...
    while current <= end:
        ...
        try:
            for i, chunk in enumerate(pd.read_sql_query(query, engine, chunksize=CHUNKSIZE)):
                ...
                rows_this_window += n
                total += n
                file_name = window_dir / f"transaction_fees_{current.date()}_chunk_{i+1}.parquet"
